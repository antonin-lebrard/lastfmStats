 ...
 * but my scope is very limited, and I would profit effectively from less than 10 percent of Angular
 * So here is a very cumbersome and very specific code to handle only my cases
 *
 * Link Html class to a Dart class that will be instantiated for each Html Element with such Html class
 */
class ComponentLinking {
  ...
  static Map<Component, bool> _isInitCalled = new Map();
  ...
  static refresh(){
    ...
    memoryLinking.values.forEach((Component c){
      _isInitCalled.putIfAbsent(c, ()=>false);
      if (!_isInitCalled[c]){
        c.onInit();
        _isInitCalled[c] = true;
      }
    });
  }
